    ...
    def pub_callback(self):
      ...
      if self.joyA==0:
        ...
        for j in range(self.sig.shape[0]):  # maior que 30 segundos todos os motores na posição do sinal 
            self.msg.id.append(j+1)
            self.msg.position.append(self.sig[j][self.i]+10)
            self.get_logger().info('Motor %d: %s' %
                                    (j+1, self.sig[j][self.i]))
        self.i += 1 #incrementa o contador de tempo 
        if (self.i >= self.sig.shape[1]): #se o contador de tempo for maior que o tamanho do sinal, reinicia o contador
            self.i = 0

      self.publisher_.publish(self.msg)      

    def sub_callback(self, data): #retorno do temporizador, executado a cada intervalo de tempo
        A, B, C, D, self.joyA = data.axes[0], data.axes[1], data.axes[3], data.axes[4], data.buttons[0]
        ...
        if self.joyA:
            ...
            # ligar motor que vai para esq
              print("esq")
              self.motors[1] += 10 * A #if self.motors[2].position < self.limite else None
              # self.motors[5] += 100 * A #if self.motors[6].position < self.limite else None
                
            elif A < 0:
            # ligar motor que vai para dir
              print("dir")
              self.motors[1] += 10 * A #if self.motors[2].position < self.limite else None
              # self.motors[3] += 100 * A #if self.motors[4].position < self.limite else None
